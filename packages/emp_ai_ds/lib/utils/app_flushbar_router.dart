...
class AppFlushbarRoute<T> extends OverlayRoute<T> {
  final AppFlushbar flushbar;
  ...
  void _configureAlignment(FlushbarPosition flushbarPosition) {
    switch (flushbar.flushbarPosition) {
      ...
      case FlushbarPosition.BOTTOMLEFT:
        {
          _initialAlignment = const Alignment(-1.0, 2.0);
          _endAlignment = flushbar.endOffset != null
              ? const Alignment(-1.0, 1.0) +
  ...
  void didReplace(Route<dynamic>? oldRoute) {
    ...
    assert(!_transitionCompleter.isCompleted,
        'Cannot reuse a $runtimeType after disposing it.');
    if (oldRoute is AppFlushbarRoute) {
  ...
  bool canTransitionTo(AppFlushbarRoute<dynamic> nextRoute) => true;

  /// Whether this route can perform a transition from the given route.
  ///
  /// Subclasses can override this method to restrict the set of routes they
  /// need to coordinate transitions with.
  bool canTransitionFrom(AppFlushbarRoute<dynamic> previousRoute) => true;
...
AppFlushbarRoute showFlushbar<T>(
    {required BuildContext context, required AppFlushbar flushbar}) {
  return AppFlushbarRoute<T>(
    flushbar: flushbar,
