...
class AppFlushbar<T> extends StatefulWidget {
  AppFlushbar(
      ...
      this.flushbarPosition = FlushbarPosition.BOTTOMLEFT,
  ...
  /// A callback for you to listen to the different AppFlushbar status
  ...
  /// [boxShadows] The shadows generated by AppFlushbar. Leave it null if you don't want a shadow.
  ...
  /// How long until AppFlushbar will hide itself (be dismissed). To make it indefinite, leave it null.
  ...
  /// Used to limit AppFlushbar width (usually on large screens)
  final double? maxWidth;

  /// Adds a custom margin to AppFlushbar
  final EdgeInsets margin;

  /// Adds a custom padding to AppFlushbar
  /// The default follows material design guide line
  final EdgeInsets padding;

  /// Adds a radius to corners specified of AppFlushbar. Best combined with [margin].
  ...
  // Adds a border to every side of AppFlushbar
  /// I do not recommend using it with [showProgressIndicator] or [leftBarIndicatorColor].
  ...
  /// AppFlushbar can be based on [FlushbarPosition.TOP] or on [FlushbarPosition.BOTTOM] of your screen.
  ...
  /// AppFlushbar can be floating or be grounded to the edge of the screen.
  ...
  /// Default is 0.0. If different than 0.0, blurs only AppFlushbar's background.
  ...
  /// Offset to be added to the end AppFlushbar position.
  /// Intended to replace [margin] when you need items below AppFlushbar to be accessible
  final Offset? endOffset;

  route.AppFlushbarRoute<T?>? flushbarRoute;
  ...
  /// When this future finishes, it is guaranteed that AppFlushbar was dismissed.
  Future<T?> dismiss([T? result]) async {
    ...
    if (flushbarRoute!.isCurrent) {
      ...
      // removeRoute is called every time you dismiss a AppFlushbar that is not the top route.
      // It will not animate back and listeners will not detect FlushbarStatus.IS_HIDING or FlushbarStatus.DISMISSED
      // To avoid this, always make sure that AppFlushbar is the top route when it is being dismissed
...
class _FlushbarState<K extends Object?> extends State<AppFlushbar<K>>
  ...
  void initState() {
    ...
    _messageTopMargin = _isTitlePresent ? 0 : widget.padding.top;
      ...
      child: Material(
        ...
        child: SafeArea(
          ...
          bottom: widget.flushbarPosition == FlushbarPosition.BOTTOM ||
              widget.flushbarPosition == FlushbarPosition.BOTTOMLEFT,
          top: widget.flushbarPosition == FlushbarPosition.TOP,
          left: widget.flushbarPosition == FlushbarPosition.BOTTOMLEFT,
          right: false,
          child: _getFlushbar(),
        ),
      ),
  ...
  EdgeInsets _getInset() {
    switch (widget.flushbarPosition) {
      ...
      case FlushbarPosition.TOP:
        return EdgeInsets.only(
          top: MediaQuery.of(context).viewInsets.top + 48,
        );
      default:
          ...
          child: Column(
            ...
            children: <Widget>[
              (_isTitlePresent)
                  ...
                  : _emptyWidget,
              Visibility(
                visible: widget.messageText != null,
                child: Padding(
                  padding: EdgeInsets.only(
      ...
      return FutureBuilder(
        ...
        builder: (BuildContext buildContext, AsyncSnapshot<Size> snapshot) {
          if (snapshot.hasData) {
            return Container(
              width: 16,
              height: snapshot.data!.height < 80 ? 80 : snapshot.data!.height,
              decoration: BoxDecoration(
...
/// [FlushbarStatus.SHOWING] AppFlushbar has stopped and the user can see it
/// [FlushbarStatus.DISMISSED] AppFlushbar has finished its mission and returned any pending values
/// [FlushbarStatus.IS_APPEARING] AppFlushbar is moving towards [FlushbarStatus.SHOWING]
/// [FlushbarStatus.IS_HIDING] AppFlushbar is moving towards [] [FlushbarStatus.DISMISSED]
