// ignore_for_file: avoid_setters_without_getters, avoid_redundant_argument_values, depend_on_referenced_packages, implementation_imports, overridden_fields
...
/// Signature for the [DataGridPager.pageItemBuilder] callback.
...
/// Signature for the builder callback used by [DataGridPager.onPageNavigationStart].
typedef PageNavigationStart = void Function(int pageIndex);

/// Signature for the [DataGridPager.onPageNavigationEnd] callback.
...
/// A controller for [DataGridPager].
///
...
///```
///
///The following code example shows how to initialize the [DataGridPager]
/// with [SfDataGrid]
///
/// ```dart
///
...
/// See also:
///
/// [DataGridPager] - which is the widget this object controls.
...
///
/// A [DataGridPager] shows[pageCount] number of pages required to display.
///
...
class DataGridPager extends StatefulWidget {
  ...
  /// The number of pages required to display in [DataGridPager].
  ...
  /// Determines the total items count
  ///
  ...
  /// the [DataGridPager] will automatically listen the listener when call the
  /// `notifyListeners` and refresh the UI based on the current configuration.
  ///
  /// This must be non-null
  final DataPagerDelegate delegate;

  /// The page to show when first creating the [DataGridPager].
  ///
...
/// A state class of a [DataGridPager] StatefulWidget that maintain the paging
/// state details.
class SfDataPagerState extends State<DataGridPager> {
  ...
  // Checks whether the `DataGridPager` is loading initially or not.
  ...
  /// Defines the text direction of [DataGridPager].
  ...
  /// Defines the theme data of [DataGridPager].
  ...
  void _removeDelegateListener(DataGridPager oldWidget) {
  ...
  /// Helps to set or remove the page count in DataGridSource when DataGridPager with
  /// SfDataGrid.
  ...
  void _ensureItems() {
    ...
    // Issue:
    //
    // FLUT-6923-DataGridPager is not working properly when updating the pageCount dynamically
    //
    // Fix:
  ...
  Widget _buildDataPagerLabel() {
    ...
    final String labelInfo =
        'Results: ${widget.totalItems > widget.rowsPerPage && index == 1 ? 1 : (widget.rowsPerPage * (index - 1)) <= 0 ? 1 : (widget.rowsPerPage * (index - 1))} - ${widget.totalItems >= widget.rowsPerPage ? widget.rowsPerPage * index : widget.totalItems} of ${widget.totalItems}';
  ...
  @override
  void didUpdateWidget(DataGridPager oldWidget) {
...
class DataPagerThemeHelper {
  ...
  /// application for the [DataGridPager] widgets.
  ///
